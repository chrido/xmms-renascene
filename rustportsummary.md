...
- Added deterministic Rust podcast integration helpers for feed/direct-stream detection, fetched RSS/Atom response handling, enclosure parsing, relative URL resolution, feed/guid/title fallbacks, playlist import with GUID/URL de-duplication, SHA-256 cache paths, cache file writes, cache freshness, TTL cleanup, cached-duration discovery, playback cache URI selection, retry-loop semantics, failed-current skip handling, and refresh scheduling, with e2e coverage.
