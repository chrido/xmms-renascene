...
- Added Rust Spotify auth/config groundwork: C-compatible API/auth constants, PKCE verifier/challenge generation, authorization URL construction, `spotify.conf` refresh-token load/save behavior, live auth-code exchange and token refresh POST handling, token response state semantics, playlist/track/device/playback-state JSON parsing, Web API playback request construction, Spotify URI playback state integration, and e2e coverage for the persisted/API/playback contracts.
- Added deterministic Rust Spotify playlist-window state and main-menu wiring, covering authentication prompts, loading/error/empty statuses, playlist selection, track display, Back/Close behavior, and importing Spotify tracks into the playlist with e2e coverage.
- Added Rust podcast integration helpers for live HTTP feed/direct-stream detection, RSS/Atom response handling, enclosure parsing, relative URL resolution, feed/guid/title fallbacks, playlist import with GUID/URL de-duplication, SHA-256 cache paths, cache file writes, cache freshness, TTL cleanup, cached-duration discovery, playback cache URI selection, retry-loop semantics, live download execution, failed-current skip handling, and refresh scheduling, with e2e coverage.
...
- Made the Rust implementation the primary Meson build/install `xmms` binary while keeping the C implementation buildable as `xmms-c` for reference during final cleanup.
...
| `rust/src/podcast.rs` | Podcast feed detection/parsing, metadata, cache, retry, and refresh helper behavior |
...
- `serde_json` for Spotify Web API response parsing.
- `ureq` for blocking podcast HTTP feed and download requests.
